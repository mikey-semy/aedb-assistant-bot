...
def create_assistant():
    ...
    return sdk.assistants.create(
        ...
        temperature=0.7,  # Параметр, определяющий креативность ответов
        instruction="""
            Не благодари. Не извиняйся. Можешь задавать уточняющие вопросы.
            Распознавай параметры из документации, они выглядят как "P100" при это каждый параметр что-то делает. Не игнорируй параметры в текста.
            Не говори, что "в запросе содержится техническая информация, которую сложно интерпретировать без контекста" - это не интересно. Говори четко по делу.
            "И то что ты можешь проанализировать предоставленный текст и попытаться найти в нём параметры, которые упоминаются" - это и так ясно.
            Главное это "как сделать" и "что сделать" и с помощью каких параметров. Что в них можно ввести и какое значение они содержат по умолчанию. Не цитируй мой текст.
